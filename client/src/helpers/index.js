...
import { EXPORT_DATA_API, LIST_USERS_API } from "../api";
import {
  IGT,
  INT,
  OCT,
...
export const additionRequest = (year, month) => {
  ...
  let UKDE_API =
    EXPORT_DATA_API + UKDET.INITIAL_ROUTE + "/" + year + "/" + month;

  return Promise.all([
    fetch(IG_API).then((res) => {
      return res.json();
    }),
    fetch(IN_API).then((res) => {
      return res.json();
    }),
    fetch(OC_API).then((res) => {
      return res.json();
    }),
    fetch(OV_API).then((res) => {
      return res.json();
    }),
    fetch(AGA_API).then((res) => {
      return res.json();
    }),
    fetch(NEX_API).then((res) => {
      return res.json();
    }),
    fetch(NSA_API).then((res) => {
      return res.json();
    }),
    fetch(NSD_API).then((res) => {
      return res.json();
    }),
    fetch(ORG_API).then((res) => {
      return res.json();
    }),
    fetch(UKDE_API).then((res) => {
      return res.json();
    }),
  ])
    .then(
      ([IGRD, INRD, OCRD, OVRD, AGARD, NEXRD, NSARD, NSDRD, ORGRD, UKDERD]) => {
        if (
          IGRD.length === 0 &&
          INRD.length === 0 &&
          OCRD.length === 0 &&
...
export const deductionRequest = (year, month) => {
  let LH_API =
    EXPORT_DATA_API + "/" + LHT.INITIAL_ROUTE + "/" + year + "/" + month;
  let MB_API =
    EXPORT_DATA_API + "/" + MBT.INITIAL_ROUTE + "/" + year + "/" + month;
  let PD_API =
    EXPORT_DATA_API + "/" + PDT.INITIAL_ROUTE + "/" + year + "/" + month;
  let NSH_API =
    EXPORT_DATA_API + "/" + NSHT.INITIAL_ROUTE + "/" + year + "/" + month;
  let GYM_API =
    EXPORT_DATA_API + "/" + GYMT.INITIAL_ROUTE + "/" + year + "/" + month;
  let SLC_API =
    EXPORT_DATA_API + "/" + SLCT.INITIAL_ROUTE + "/" + year + "/" + month;
  let SLD_API =
    EXPORT_DATA_API + "/" + SLDT.INITIAL_ROUTE + "/" + year + "/" + month;
  let NSHD_API =
    EXPORT_DATA_API + "/" + NSHDT.INITIAL_ROUTE + "/" + year + "/" + month;

  return Promise.all([
    fetch(LH_API).then((res) => {
      return res.json();
    }),
    fetch(MB_API).then((res) => {
      return res.json();
    }),
    fetch(PD_API).then((res) => {
      return res.json();
    }),
    fetch(NSH_API).then((res) => {
      return res.json();
    }),
    fetch(GYM_API).then((res) => {
      return res.json();
    }),
    fetch(SLC_API).then((res) => {
      return res.json();
    }),
    fetch(SLD_API).then((res) => {
      return res.json();
    }),
    fetch(NSHD_API).then((res) => {
      return res.json();
    }),
  ])
...
export const fetchDataRequest = (year, month, teamName) => {
  let SEARCH_API =
    EXPORT_DATA_API + "/" + _.kebabCase(teamName) + "/" + year + "/" + month;
